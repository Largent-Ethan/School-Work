...
void list_free(struct list* list) {
  struct link* nx_ptr = list->head->next;
  while(list->head->next != NULL){
    free(list->head);
    list->head = nx_ptr;
    nx_ptr = list->head->next;
  }
  free(list->head);
  free(list);
  return;
...
void list_insert(struct list* list, void* val) {
  struct link* hd_ptr = list->head;
  list->head = malloc(sizeof(struct link));
  list->head->val = val;
  list->head->next = hd_ptr;
  return;
}
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
  struct link* rm_ptr = list->head;
  struct link* prev = list->head;
  if(rm_ptr == NULL){
    return;
  }
  if(cmp(val, rm_ptr->val) == 0){
    list->head = rm_ptr->next;
    free(rm_ptr);
    return;
  }
  while(rm_ptr != NULL){
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
  int index = 0;
  struct link* i_ptr = list->head;
  while(cmp(val, i_ptr->val) != 0){
    i_ptr = i_ptr->next;
    if(i_ptr == NULL)
      return -1;
    index = index + 1;
...
void list_reverse(struct list* list) {
  if(list->head == NULL)
    return;
  struct link* new_hd = list->head;
  struct link* nxt_hd = new_hd->next;
  struct link* nn_hd;
  new_hd->next = NULL;
  while(nxt_hd != NULL){
    nn_hd = nxt_hd->next;
